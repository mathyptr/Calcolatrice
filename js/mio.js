	function jsGet(exp){
		var xhttp = new XMLHttpRequest();
			...
			xhttp.open("GET"," http://api.mathjs.org/v4/?expr="+exp, true);
			xhttp.send();
	}

	function jqueryGet(exp){
		$.get( "http://api.mathjs.org/v4/?expr="+exp, function( data ) {
			document.getElementById("operazioni").value = data; 
			});
	}
	...
	jQuery.ajax ({
			...
			success: function(data,status){
			 document.getElementById("operazioni").value = data.result;
			}
		});
	}
